...
public class RTreeLeafNode extends RTreeNode implements Serializable{
	...
	**/
	...
	public RTreeLeafNode(int n,String strTableName,String strColName) 
	{
		super(n);
		this.strTableName = strTableName;
		this.strColName = strColName;
		keys = new comparablePolygon[n];
	 ...
	 */
	public PushUp insert(comparablePolygon key, Ref recordReference, RTreeInnerNode parent, int ptr)
	{
		...
		if(this.isFull())
		{
			...
			comparablePolygon newKey = newNode.getFirstKey();
	...
	private int keyIndex(comparablePolygon key) {
	 ...
	 */
	private void insertAt(int index, comparablePolygon key, String pointer) 
	{
	 ...
	 */
	public RTreeNode split(comparablePolygon key, Ref recordReference) 
	{
		...
		RTreeLeafNode newNode = new RTreeLeafNode(order,this.strTableName,this.strColName);
	 ...
	 */
	public int findIndex(comparablePolygon key) 
	{
		for (int i = 0; i < numberOfKeys; ++i) 
		{
	 ...
	 */
	public boolean delete(comparablePolygon key, RTreeInnerNode parent, int ptr) 
	{
		for(int i = 0; i < numberOfKeys; ++i)
			if(keys[i].compareTo(key) == 0)
			{
	 ...
	 */
	public void deleteRef(comparablePolygon key, RTreeInnerNode parent, int ptr, Ref recordReference) 
	{
		for(int i = 0; i < numberOfKeys; ++i)
			if(keys[i].compareTo(key) == 0)
			{
	...
	public Iterator<Vector<Object>> allTuplesSmallerThan(comparablePolygon key,int indexOfKey) {
	...
	public Iterator<Vector<Object>> allTuplesSmallerThanOrEqual(comparablePolygon key,int indexOfKey) {
		LinkedList<Vector<Object>>list=new LinkedList<>();
		boolean end=false;
		for(int i=0;i<numberOfKeys;i++) 
		{
	...
	public RTreeLeafNode searchLeafNode(comparablePolygon key) 
	{
		return this;
		
	}
	
	public Iterator<Vector<Object>> allTuplesgreaterThanOrEqual(comparablePolygon key,int indexOfKey) {
		LinkedList<Vector<Object>>list=new LinkedList<>();
		for(int i=0;i<numberOfKeys;i++) 
		{
			if(getKey(i).compareTo(key)<0) 
	...
	public Iterator<Vector<Object>> allTuplesgreaterThan(comparablePolygon key,int indexOfKey) {
		LinkedList<Vector<Object>>list=new LinkedList<>();
		for(int i=0;i<numberOfKeys;i++) 
		{
			if(getKey(i).compareTo(key)<=0) 
